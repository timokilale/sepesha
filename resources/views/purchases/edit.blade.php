  ...
  <div class="bg-white rounded shadow p-6">
    <form method="POST" action="{{ route('purchases.update', $purchase) }}" class="space-y-4">
      ...
      <div>
        ...
        <input name="item_name" value="{{ old('item_name', $purchase->item_name) }}" class="w-full border rounded px-3 py-2" required />
      </div>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label class="block text-sm text-gray-700 mb-1">Cost price</label>
          <input type="number" step="0.01" name="cost_price" value="{{ old('cost_price', $purchase->cost_price) }}" class="w-full border rounded px-3 py-2" required />
        </div>
        <div>
          <label class="block text-sm text-gray-700 mb-1">Quantity</label>
          <input type="number" min="1" name="quantity" value="{{ old('quantity', $purchase->quantity) }}" class="w-full border rounded px-3 py-2" required />
        </div>
        <div>
